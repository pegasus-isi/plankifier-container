...
class Ctrain:
    ...
    def ReadArgs(self, string=None):
        ...
        parser = argparse.ArgumentParser(
            description="Train different models on plankton images"
        )
        ...
        parser.add_argument(
            ...
            nargs="*",
            default=[
                "./data/1_zooplankton_0p5x/training/zooplankton_trainingset_2020.04.28/"
            ],
        ...
        parser.add_argument(
            "-outpath",
            default="./out/",
            help="directory where you want the output saved",
        )
        parser.add_argument(
            "-load_weights", default=None, help="Model weights that should be loaded."
        )
        parser.add_argument(
            "-saveModelName",
            default="keras_model.h5",
            help="Name of the model when it is saved at the end of the run.",
        )
        # User experience
        parser.add_argument(
            "-verbose", action="store_true", help="Print many messages on screen."
        )
        parser.add_argument(
            "-plot",
            action="store_true",
            help="Plot loss and accuracy during training once the run is over.",
        )
        # Hyperparameters
        parser.add_argument(
            "-opt",
            choices=["sgd", "adam"],
            default="sgd",
            help="Choice of the minimization algorithm (sgd,adam)",
        )
        ...
        parser.add_argument(
            "-modelfile",
            default=None,
            help="The name of the file where a model is stored (to be loaded with keras.models.load_model() )",
        )
        parser.add_argument(
            "-model_image",
            choices=[
            ...
            default=None,
            help="For mixed data models, tells what model to use for the image branch. For image models, it is the whole model",
        )
        parser.add_argument(
            "-model_feat",
            choices=["mlp"],
            default=None,
            help="For mixed data models, tells what model to use for the feature branch. For feat models, it is the whole model.",
        )
        parser.add_argument(
            "-layers", nargs=2, type=int, default=[256, 128], help="Layers for MLP"
        )
        # Data
        parser.add_argument(
            "-L",
            type=int,
            default=128,
            help="Images are resized to a square of LxL pixels",
        )
        parser.add_argument(
            "-testSplit",
            type=float,
            default=0.2,
            help="Fraction of examples in the test set",
        )
        parser.add_argument(
            "-class_select",
            nargs="*",
            default=None,
            help="List of classes to be looked at (put the class names one by one, separated by spaces). If None, all available classes are studied.",
        )
        parser.add_argument(
            "-classifier",
            choices=["binary", "multi", "versusall"],
            default="multi",
        ...
        parser.add_argument(
            ...
            choices=["yes", "no"],
            default="no",
            help='Choose "yes" or "no" for scheduling learning rate',
        )
        parser.add_argument(
            "-balance_weight",
            choices=["yes", "no"],
        ...
        parser.add_argument(
            ...
            type=int,
            default=5,
            help="Total number of epochs for the training",
        )
        parser.add_argument(
            "-initial_epoch", type=int, default=0, help="Initial epoch of the training"
        )
        parser.add_argument(
            "-earlyStopping",
            type=int,
            default=100,
            help="If >0, we do early stopping, and this number is the patience (how many epochs without improving)",
        )
        ...
        parser.add_argument(
            ...
            nargs="*",
            default="mobile",
            help="select models to train from: conv,mobile,eff0,eff1,eff2,eff3,eff4,eff5,eff6,eff7,incepv3,res50,dense121",
        )
        # Ensemble related
        parser.add_argument(
            "-stacking_ensemble",
            choices=["yes", "no"],
            default="no",
            help='Choose "yes" or "no" for running Stacking ensemble',
        )
        parser.add_argument(
            "-avg_ensemble",
            choices=["yes", "no"],
            default="no",
            help='Choose "yes" or "no" for running Average ensemble ',
        )
        parser.add_argument(
            "-finetune", type=int, default=0, help='Choose "0" or "1" for finetuning'
        )
        parser.add_argument(
            "-finetune_epochs",
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=5,
            help="Total number of trials for hyperparameter optimization",
        )
        parser.add_argument(
            "-epochs",
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help="Total number of trials for each hyperparameter optimization trial",
        )
        parser.add_argument(
            "-bayesian_epoch",
            type=int,
        ...
        parser.add_argument(
            ...
            choices=["yes", "no"],
            default=None,
            help="Whether to load saved data or not",
        )
        parser.add_argument(
            "-save_data",
            choices=["yes", "no"],
        ...
        parser.add_argument(
            ...
            choices=["yes", "no"],
            default=None,
            help="Whether to train only ensemble models",
        )
        parser.add_argument(
            "-mixed_from_finetune",
            type=int,
            default=0,
            help="Whether to train mixed models using finetuned image and feature models",
        )
        parser.add_argument(
            "-mixed_from_notune",
            type=int,
            default=0,
            help="Whether to train mixed models using Not tuned image and feature models",
        )
        parser.add_argument(
            "-mixed_from_scratch",
            type=int,
            default=0,
            help="Whether to train mixed models using image and feature models from scratch",
        )
        parser.add_argument(
            "-valid_set",
            choices=["yes", "no"],
    ...
    def ArgsCheck(self, args):
        ...
        if args.L < 8:
            raise ValueError(
                "Linear size of the images <8 pixels is too small to be wanted. Abort."
            )

        if args.initial_epoch >= args.totEpochs:
            print(
                "The initial epoch is already equal or larger than the target number of epochs, so there is no need to do anything. Exiting..."
            )
            raise SystemExit
        ...
        if (
            args.model_image is None
            and args.model_feat is None
            and args.hp_tuning is None
        ):
            ...
            if self.verbose:
                print(
                    "No model was specified by the user, so we analyze images with {}".format(
                        args.model_image
                    )
                )
        ...
        if args.ttkind != "image" and args.aug == True:
            print(
                "User asked for data augmentation, but we set it to False, because we only do it for `image` models"
            )
            args.aug = False
        ...
        if args.ttkind == "image":
            ...
            print(
                "User asked for computing extra features, but we set it to False, because we only do it for `mixed` models"
            )
    ...
    def LoadData(
        ...
        # Default values
        if datapaths == None:
            datapaths = self.params.datapaths
        if L == None:
            L = self.params.L
        if class_select == -1:
            class_select = self.params.class_select  # class_select==None has the explicit meaning of selecting all the classes
        if classifier == None:
            classifier = self.params.classifier
        if compute_extrafeat == None:
            compute_extrafeat = self.params.compute_extrafeat
        if resize_images == None:
            resize_images = self.params.resize_images
        if balance_weight == None:
            balance_weight = self.params.balance_weight
        if datakind == None:
            datakind = self.params.datakind
    ...
    def CreateTrainTestSets(
        ...
        if self.params.ttkind == "mixed":
            ...
            if self.params.valid_set == "yes":
                ...
                Data = [
                    self.tt.trainFilenames,
                    self.tt.trainXimage,
                    self.tt.trainY,
                    self.tt.testFilenames,
                    self.tt.testXimage,
                    self.tt.testY,
                    self.tt.valFilenames,
                    self.tt.valXimage,
                    self.tt.valY,
                    self.tt.trainXfeat,
                    self.tt.testXfeat,
            ...
            dump(
                scaler,
                self.params.outpath + "/Aqua_Features_scaler_used_for_MLP.joblib",
            )
            self.tt.trainXfeat = scaler.transform(self.tt.trainXfeat)
            self.tt.testXfeat = scaler.transform(self.tt.testXfeat)
            if self.params.valid_set == "yes":
        ...
        with open(
            self.params.outpath + "/Filenames_for_Ensemble_training.pickle", "wb"
        ) as b:
    ...
    def Train(self, train=True):
        if self.params.hp_tuning is None:  # if hyperparameter tuning is not chosen
            ...
            checkpointer = keras.callbacks.ModelCheckpoint(
                filepath=self.params.outpath + "/bestweights.hdf5",
                monitor="val_loss",
                verbose=0,
                save_best_only=True,
            )  # save the model at every epoch in which there is an improvement in test accuracy
            logger = keras.callbacks.CSVLogger(
                self.params.outpath + "/epochs.log", separator=" ", append=False
            )
            ...
            if self.params.saved_data is None:
                ...
                if train == False:
                    ...
                    trX, trY, teX, teY, class_weights = (
                        [self.tt.trainXimage, self.tt.trainXfeat],
                        self.tt.trainY,
                        [self.tt.testXimage, self.tt.testXfeat],
                        self.tt.testY,
                        self.tt.class_weights,
                    )
                else:
                ...
                wrapper = hm.CModelWrapper(
                    ...
                    class_weights,
                    self.trainParams,
                    numclasses=len(self.data.classes),
                )
                ...
                if self.params.balance_weight == "yes":
                    class_weights = pd.read_pickle(
                        self.params.outpath + "/class_weights.pickle"
                    )
                ...
                if self.params.valid_set == "no":
                    if self.params.ttkind == "mixed":
                        ...
                        self.params.ttkind == "image"
                        and self.params.compute_extrafeat == "yes"
                    ):
                        trainFilenames = Data[0]
                        trainXimage = Data[1]
                        trY = Data[2]
            ...
            if (
                self.params.classifier == "binary"
                or self.params.classifier == "versusall"
            ):
            ...
            if self.params.ttkind == "image":
                if (
                    self.params.only_ensemble == "no"
                    or self.params.only_ensemble is None
                ):
                    for i in range(len(self.params.models_image)):
                        hm.get_and_train_best_models(
                            X_train=trainX,
                            y_train=trainY,
                            X_test=testX,
                            y_test=testY,
                ...
                if (
                    ...
                    if self.params.mixed_from_scratch == 1:
                        ...
                        for i in range(len(self.params.models_image)):
                            ...
                            members = hm.Select_Mixed_Model_from_scratch(
                                X_train, classes, self.params.models_image[i]
                            )
                            Mixed_model = hm.define_Mixed_model_from_scratch(
                                members, len(classes), self.loss
                            )
                            Mixed_model_name = (
                                "FromScratchMixed_"
                            ...
                            hm.compile_and_train(
                                Mixed_model,
                                self.params.outpath,
                                Mixed_model_name,
                                1e-05,
                                [trainXimage, trainXfeat],
                                trainY,
                                [testXimage, testXfeat],
                                testY,
                                [valXimage, valXfeat],
                                valY,
                                self.params.epochs,
                        ...
                        if self.params.avg_ensemble == "yes":
                            if len(Mixed_models_name) > 1:
                                ...
                                print(
                                    "For Ensemble, models selected should be greater than 1"
                                )
                    ...
                    if (
                        self.params.mixed_from_notune == 1
                        or self.params.mixed_from_finetune == 1
                    ):
                        ...
                        feature_filepath = (
                            self.params.outpath
                            + "BestModelsFromBayesianSearch/"
                            + self.params.init_name
                            + "/Feature/mlp"
                        )
                    ...
                    if self.params.mixed_from_notune == 1:
                        ...
                        for i in range(len(self.params.models_image)):
                            ...
                            Mixed_model = hm.define_Mixed_model(
                                members, len(classes), self.loss
                            )
                            Mixed_model_name = (
                                "FromNotuneMixed_"
    ...
    def Report(self):
        ...
        clrep = classification_report(
            self.tt.testY.argmax(axis=1),
    ...
    def IdentifyWorsePrediction(self, predictions):
        ...
        for i in range(len(self.tt.testY)):
            ...
            if (
                self.tt.testY.argmax(axis=1)[i] == predictions.argmax(axis=1)[i]
            ):  # correct classification
                if (
                    predictions[i][predictions[i].argmax()] > maxconf_right
                ):  # if the confidence on this prediction is larger than the largest seen until now
                    i_maxconf_right = i
    ...
    def Confidences(self, predictions):
        ...
        confidences = predictions.max(
            axis=1
        )  # confidence of each prediction made by the classifier
        whether = np.array(
            [
                1 if testY.argmax(axis=1)[i] == predictions.argmax(axis=1)[i] else 0
                for i in range(len(predictions))
            ]
        )  # 0 if wrong, 1 if right
        self.confidences_right = confidences[
            np.where(testY.argmax(axis=1) == predictions.argmax(axis=1))[0]
        ]
        self.confidences_wrong = confidences[
            np.where(testY.argmax(axis=1) != predictions.argmax(axis=1))[0]
        ]
    ...
    def Abstention(self, thresholds=[0.999], print=True, save=True, plot=False):
        ...
        accs, nconfident = (
            np.ndarray(len(thresholds), dtype=np.float),
            np.ndarray(len(thresholds), dtype=np.int),
        )
        for i, thres in enumerate(thresholds):
            confident = np.where(confidences > thres)[0]
            nconfident[i] = len(confident)
            accs[i] = (
                whether[confident].sum() / nconfident[i]
                if nconfident[i] > 0
                else np.nan
            )
    ...
    def SaveModel(self):
        self.model.save(
            self.params.outpath + "/" + self.params.saveModelName,
            overwrite=True,
