...
class Cpred:
    ...
    def __init__(
        self,
        modelname="./keras_model.h5",  # modelpath='./',
        weightsname="./bestweights.hdf5",
        verbose=False,
    ):
        # Set class variables
    ...
    def PredictionBundle(self, npimages):
        ...
        # Predictions
        self.predictions = self.probs.argmax(
            axis=1
        )  # The class that the classifier would bet on
        self.confidences = self.probs.max(
            axis=1
        )  # equivalent to: [probs[i][predictions[i]] for i in range(len(probs))]
        self.predictions_names = np.array(
            [self.classes[self.predictions[i]] for i in range(nimages)], dtype=object
        )

        # Classifier's second choice
        self.predictions2 = self.probs.argsort(axis=1)[
            :, -2
        ]  # The second most likely class
        self.confidences2 = np.array(
            [self.probs[i][self.predictions2[i]] for i in range(nimages)], dtype=float
        )
        self.predictions2_names = np.array(
            [self.classes[self.predictions2[i]] for i in range(nimages)], dtype=object
        )
    ...
    def LoadModel(self):
        ...
        try:
            self.simPred.params = np.load(
                self.modelpath + "/params.npy", allow_pickle=True
            ).item()
            ...
            try:
                ...
                self.simPred.params = np.load(
                    str(p) + "/params.npy", allow_pickle=True
                ).item()
                print("LOADED : ", str(p) + "/params.npy")
            except:
                print("WARNING: We were not able to load ", str(p) + "/params.npy")

        self.simPred.LoadModel(
            self.modelname, self.weightsname
        )  # Loads models and updates weights
...
class Censemble:
    ...
    def __init__(
        ...
        self.weightnames = (
            weightnames
            if (len(modelnames) == len(weightnames))
            else np.full(len(modelnames), weightnames)
        )  # Usually, the file with the best weights of the run
        ...
        sizes = list(
            set([self.predictors[im].simPred.params.L for im in range(self.nmodels)])
        )
        ...
        for L in sizes:
            ...
            # 			self.npimages[L] = hd.LoadImageList(self.im_names, L, show=False) # Load images
            self.npimages[L] = hd.LoadImageList(
                self.im_names, L, True, show=False
            )  # Load
            counti = counti + 1
        ...
        for imn, mname in enumerate(self.modelnames):
            ...
            if 0 == imn:
                ...
                assert np.all(classnames == predictors[imn].classes), (
                    "Class names for different models do not coincide"
                )
    ...
    def GetImageNames(self, training_data=False):
        ...
        for itd, td in enumerate(self.testdirs):
            if not os.path.isdir(td):
                print(
                    "You told me to read the images from a directory that does not exist:\n{}".format(
                        td
                    )
                )
                raise IOError

            im_names_here = np.array(
                glob.glob(td + training_data_string + "/*.jpeg"), dtype=object
            )
        ...
        return np.array(all_names), (
            None if self.labels == None else np.array(all_labels)
        )
    ...
    def Majority(self, predictions):
        if len(set(predictions)) == 0:
            ...
            amax = np.argsort(list(counter.values()))[
                -1
            ]  # Index (in counter) of the highest value
            amax2 = np.argsort(list(counter.values()))[
                -2
            ]  # Index of second highest value

            if (
                list(counter.values())[amax] == list(counter.values())[amax2]
            ):  # there is a tie
                return __UNCLASSIFIED__
            else:
    ...
    def Ensemble(self, method, absthres):
        ...
        if method == None:
            method = self.ensMethod
        if absthres == None:
            absthres = self.absthres

        self.guesses = []
        for iim, im_name in enumerate(self.im_names):
            predictions = np.array(
                [self.predictors[imod].predictions[iim] for imod in range(self.nmodels)]
            )
            confidences = np.array(
                [self.predictors[imod].confidences[iim] for imod in range(self.nmodels)]
            )
            pr = np.array(
                [self.predictors[imod].pr[iim] for imod in range(self.nmodels)]
            )

            # Abstention
            if absthres > 0 and (method != "weighted-majority"):
                predictions, confidences, pr = self.Abstain(
                    predictions, confidences, pr, thres=absthres, metric="prob"
                )
            ...
            if method == "unanimity":  # Predict only if all models agree
                ...
                method == "leader"
            ):  # Follow-the-leader: only give credit to the most confident prediction
            ...
            ):  # Weighted Majority (like majority, but each is weighted by their confidence)
                guess = (
                    im_name,
                    self.WeightedMajority(predictions, confidences, absthres),
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Load a model and use it to make predictions on images"
    )
    parser.add_argument(
        "-modelfullnames",
        nargs="+",
    ...
    parser.add_argument(
        "-weightnames",
        nargs="+",
        default=["./trained-models/conv2/bestweights.hdf5"],
        help="Name of alternative weights for the model. Must include path",
    )
    parser.add_argument(
        "-testdirs",
        nargs="+",
    ...
    parser.add_argument(
        "-predname",
        default="./out/predictions/predict",
        help="Name of the file with the model predictions (without extension)",
    )
    parser.add_argument(
        "-verbose",
        action="store_true",
        help="Print lots of useless tensorflow information",
    )
    parser.add_argument(
        "-ensMethods",
        nargs="+",
        default=["unanimity"],
        help="Ensembling methods. Choose from: 'unanimity','majority', 'leader', 'weighted-majority'. Weighted Majority implements abstention in a different way (a good value is 1).",
    )
    parser.add_argument(
        "-thresholds",
        nargs="+",
        default=[0],
        type=float,
        help="Abstention thresholds on the confidence (a good value is 0.8, except for weighted-majority, where it should be >=1).",
    )
    parser.add_argument(
        "-nosuffix",
        action="store_true",
        help="If activated, no suffix is added to the output file",
    )
    ...
    ensembler = Censemble(
        modelnames=args.modelfullnames,
        testdirs=args.testdirs,
        weightnames=args.weightnames,
    )
    ...
    for method in args.ensMethods:
        for absthres in args.thresholds:
            ...
            if args.nosuffix:
                ...
                ensembler.WriteGuesses(
                    "{}_{}abs{}.txt".format(args.predname, method, absthres)
                )
